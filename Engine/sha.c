...
static const unsigned long sha_const_key[5]=
{
  0x67452301,
  0xEFCDAB89,
  0x98BADCFE,
                      ...
                      byte Message_Digest[SHA1_HASH_SIZE])
{
  ...
  assert(context!=NULL && Message_Digest!=NULL);
...
retCode g_sha1(processPo P,ptrPo a)
{
  ...
  if(isvar(Data))
    ...
    ptrI hash = allocateByteList(&P->proc.heap,sha_result,SHA1_HASH_SIZE);
